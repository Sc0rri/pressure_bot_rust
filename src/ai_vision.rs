...
impl AiVisionService {
    ...
    pub async fn recognize_pressure_batch(env: &Env, image_bytes: &[u8]) -> Result<Vec<String>> {
        let retries_str = get_env_or_secret(env, "AI_VISION_RETRIES", &DEFAULT_VISION_RETRIES.to_string());
        ...
        for i in 0..retries {
            ...
            handles.push(async move {
                ...
                if status != 200 {
                    ...
                    return (i, Err(worker::Error::from(format!("AI API error ({}): {}", status, body))));
                }

                let ai_response: serde_json::Value = match serde_json::from_str(&body) {
                    Ok(v) => v,
                    Err(e) => return (i, Err(worker::Error::from(format!("AI JSON parse error: {}", e)))),
                };
        ...
        console_log!("AI batch complete: {} successful responses out of {} attempts", results.len(), retries);
